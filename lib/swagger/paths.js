...
internals.paths.prototype.build = function (routes) {
  ...
  routes.forEach((route) => {
    const config = Hoek.reach(route, 'config') || {}
    // let routeOptions = Hoek.reach(route, 'settings.plugins.hapi-swagger') || {};
    let routeData = {
      path: route.path,
      method: route.method.toUpperCase(),
      description: Hoek.reach(config, 'description'),
      summary: Hoek.reach(config, 'summary'),
      tags: Hoek.reach(config, 'tags'),
      queryParams: Hoek.reach(config, 'validate.query'),
      pathParams: Hoek.reach(config, 'validate.params'),
      payloadParams: Hoek.reach(config, 'validate.payload'),
      responseSchema: Hoek.reach(config, 'response.schema'),
      responseStatus: Hoek.reach(config, 'response.status'),
      headerParams: Hoek.reach(config, 'validate.headers'),
    ...
    // TODO: hapi-swagger的routeOptions处理，此处可做优化，理论上swagi不需要很多的选项（有待验证）
    // user configured interface through route plugin options
    if (Hoek.reach(config, 'validate.query')) {
      routeData.queryParams = Utilities.toJoiObject(Hoek.reach(config, 'validate.query'))
    }
    if (Hoek.reach(config, 'validate.params')) {
      routeData.pathParams = Utilities.toJoiObject(Hoek.reach(config, 'validate.params'))
    }
    if (Hoek.reach(config, 'validate.headers')) {
      routeData.headerParams = Utilities.toJoiObject(Hoek.reach(config, 'validate.headers'))
    }
    if (Hoek.reach(config, 'validate.payload')) {
      // has different structure, just pass straight through
      routeData.payloadParams = Hoek.reach(config, 'validate.payload')
...
internals.paths.prototype.buildRoutes = function (routes) {
  ...
  routes.forEach((route) => {
    ...
    if (this.settings.grouping === 'tags') {
      if (!route.tags) {
        throw Error(`expect tags defined on route ${route.path} when swagger is group by tags but got ${route.tags}`)
      }
    ...
    out.description = Array.isArray(route.description) ? route.description.join('<br/><br/>') : route.description
