...
internals.getSchema = function(ctx) {
    const forwardedProtocol = ctx.headers['x-forwarded-proto'];

    if (forwardedProtocol) {
        return forwardedProtocol;
    }

    // Azure Web Sites adds this header when ctxs was received via HTTPS.
    if (ctx.headers['x-arr-ssl']) {
        return 'https';
    }
    ...
    // receives ctx over HTTP and passes it to node via a named pipe.
    if (protocol === 'socket') {
        return 'http';
    }
